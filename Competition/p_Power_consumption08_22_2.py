...
BASE_PATH = "C:\Study25\\"# if os.path.exists("/workspace/TensorJae/Study25/") \
    # else os.path.expanduser("~/Desktop/IBM:RedHat/Study25/")
path = os.path.join(BASE_PATH, "_data\dacon\Power_consumption\\")
...
def generate_hourly_dates(start_date, end_date):
    ...
    while current <= end:
        for hour in range(24):
            dates.append(current.strftime("%Y%m%d") + f" {hour:02d}")
        current += timedelta(days=1)
    return dates

# ✂️ 삭제 대상 입력 (날짜 형식으로)
# 날짜 한 개만 있는 경우 → 하루 24시간
...
# === 1) 공통 시간 파생
# [PATCH] 토/일/월 분리 플래그 추가(is_saturday, is_sunday)
...
for idx in train.index:
    # 조건: 건물번호가 대상에 포함되고, target 값이 0.0일 때
    if train.loc[idx, '건물번호'] in target_buildings and train.loc[idx, target] == 0.0:
        # 다음 인덱스가 존재할 경우
        if idx + 1 in train.index:
            train.loc[idx, target] = train.loc[idx + 1, target]
...
# def objective_lasso_on_oof(trial, oof_meta, y_full):
#     alpha = trial.suggest_float("alpha", 1e-4, 10.0, log=True)
#     lasso = Lasso(alpha=alpha)
#     kf = KFold(n_splits=5, shuffle=True, random_state=seed)
#     scores = []
#     for tr_idx, va_idx in kf.split(oof_meta):
#         lasso.fit(oof_meta[tr_idx], y_full[tr_idx])
#         preds = lasso.predict(oof_meta[va_idx])
#         scores.append(smape_exp(y_full[va_idx], preds))
#     return float(np.mean(scores))
# ------------------------------
...
def process_building_kfold(bno):
    ...
    'cat': {
        "iterations": 15897,
        "learning_rate": 0.05085069088295181,
        "depth": 6,
        "l2_leaf_reg": 19.992646093088798,
        "bagging_temperature": 1.990748602144159,
        "random_strength": 1.5610493500056912
    },
    'lgb': {
        "n_estimators": 8995,
        "learning_rate": 0.06425080436422352,
        "num_leaves": 506,
        "max_depth": 8,
        "subsample": 0.8056250189621275,
        "subsample_freq": 3,
        "colsample_bytree": 0.7096613893633626,
        "min_child_samples": 10,
        "min_split_gain": 0.27398967835051147,
        "reg_alpha": 0.41483329230407484,
        "reg_lambda": 2.6843720029976312
    },
    'ridge': {
        "alpha": 0.29930672098532624
    },
    'xgb': {
        "n_estimators": 4701,
        "learning_rate": 0.06434039755833895,
        "max_depth": 10,
        "min_child_weight": 4.218069280691452,
        "gamma": 0.006526112584430506,
        "subsample": 0.7309758961217238,
        "colsample_bytree": 0.7908471806625494,
        "reg_alpha": 2.962180040227103e-06,
        "reg_lambda": 4.111231726100426
    }
    }
    ...
    best_twd={
        'twd': {
        "n_estimators": 9458,
        "learning_rate": 0.10878929479418055,
        "num_leaves": 681,
        "max_depth": 4,
        "subsample": 0.7531574170964376,
        "subsample_freq": 5,
        "colsample_bytree": 0.8780965340742359,
        "min_child_samples": 30,
        "min_split_gain": 0.42085621856679933,
        "reg_alpha": 0.28096879724368423,
        "reg_lambda": 27.348023513834548,
        "tweedie_variance_power": 1.4658936080473421
    }
    }
    ...
    # X_test = te_b[features].values
        ...
        # # '건물번호'가 9인 경우 처리
    # if (train['건물번호'] == 9).any():  # 9인 데이터가 하나라도 있으면
    #     hotel_data = train[train['건물번호'] == 9]
    #     hotel_data = hotel_data[hotel_data['month'] != 6]
        
    #     X_full = hotel_data[features].values
    #     y_full_log = np.log1p(tr_b[target].values.astype(float))
    #     X_test = te_b[features].values

    # '건물번호'가 3인 경우 처리
    # elif (train['건물번호'] == 3).any():  # 3인 데이터가 하나라도 있으면
    #     department_store_data = train[train['건물번호'] == 3]
    #     department_store_data = department_store_data[department_store_data['month'] == 8]
        
    #     X_full = department_store_data[features].values
    #     y_full_log = np.log1p(tr_b[target].values.astype(float))
    #     X_test = te_b[features].values
    ...
    #     y_full_log = np.log1p(tr_b[target].values.astype(float))
    #     X_test = te_b[features].values
    print('X_full:',tr_b[features].columns)
    print('X_test:',te_b[features].columns)  # 또는 예측에 사용되는 데이터
    # exit()
    
    # 외부 KFold TimeSeriesSplit
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X_full), 1):
        ...
        # Tweedie (원시 타깃)
        twd = LGBMRegressor(**best_twd["twd"])
    ...
    meta = Ridge(alpha=best_params["ridge"]["alpha"])
...
# cat :{"iterations": 15897, "learning_rate": 0.05085069088295181, "depth": 6, "l2_leaf_reg": 19.992646093088798, "bagging_temperature": 1.990748602144159, "random_strength": 1.5610493500056912}
# lgb :{"n_estimators": 8995, "learning_rate": 0.06425080436422352, "num_leaves": 506, "max_depth": 8, "subsample": 0.8056250189621275, "subsample_freq": 3, "colsample_bytree": 0.7096613893633626, "min_child_samples": 10, "min_split_gain": 0.27398967835051147, "reg_alpha": 0.41483329230407484, "reg_lambda": 2.6843720029976312}
# ridge :{"alpha": 0.29930672098532624}
