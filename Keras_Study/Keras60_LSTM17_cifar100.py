...
x_test = (x_test - 127.5) / 127.5 # => relu에서 음수는 소멸될 수 있다.
...
#2. 모델 구성
model = Sequential()
model.add(LSTM(256, input_shape=(32,96), activation='relu'))
...
model.add(BatchNormalization())
model.add(Dropout(0.2))
model.add(Dense(14,  activation='relu'))#XGBoost가 받아야 하는 feature 수는 훈련할 때의 입력 특성 수와 동일해야 합니다.
model.add(BatchNormalization())
...
es = EarlyStopping(monitor='val_loss',mode='min',patience=70,verbose=1,
...
import time
start = time.time()
hist = model.fit(x_train,y_train, epochs= 1, batch_size= 1024,verbose=2,validation_split=0.1, callbacks=[es])
end = time.time()
#4. 평가 예측
...
# loss : 3.31549334526062
# acc : 0.21490000188350677
# 시간 : 125.226

# loss : 4.5887041091918945
# acc : 0.014800000004470348
# 시간 : 20.667