...
model = Sequential()
model.add(Conv2D(1000,(2,2),input_shape=(32,32,3), activation='relu'))
model.add(MaxPooling2D())
model.add(BatchNormalization())
model.add(Dropout(0.3))
model.add(Conv2D(filters=1000,kernel_size=(2,2), activation='relu'))
model.add(BatchNormalization())
model.add(Dropout(0.3))
...
model.add(Dropout(0.3))
model.add(Flatten())
model.add(Dense(100000,  activation='relu'))#XGBoost가 받아야 하는 feature 수는 훈련할 때의 입력 특성 수와 동일해야 합니다.
model.add(BatchNormalization())
...
hist = model.fit(x_train,y_train, epochs= 1000, batch_size= 1000,verbose=2,validation_split=0.1, callbacks=[es,mcp])
#4. 평가 예측
