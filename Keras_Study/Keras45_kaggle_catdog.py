...
#start = time.time()
x_train = np.load(np_path+"Keras44_01_x_train100.npy")
y_train = np.load(np_path+"Keras44_01_y_train100.npy")
x_test = np.load(np_path+"Keras44_01_x_test100.npy")
y_test = np.load(np_path+"Keras44_01_y_test100.npy")
#end = time.time()
...
filename = '.hdf5'
filepath = "".join([path,'k44_',filename])
...
hist = model.fit(x_train1,y_train1, epochs= 200, batch_size= 30,verbose=2,validation_split=0.05, callbacks=[es,mcp])
#4. 평가 예측
