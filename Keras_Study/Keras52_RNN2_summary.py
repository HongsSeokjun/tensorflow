...
#2. 모델구성
model = Sequential()
# model.add(SimpleRNN(10, input_shape=(3,1),activation='relu')) # 3차원이지만 아웃풋이 2차원이 된다는 점
...
# **기존 RNN과 다르게, 셀 상태(cell state)**라는 별도의 ‘기억통로’를 둬서
#→ 중요한 정보를 오래 기억하고, 필요 없으면 잊도록 설계함

model.add(GRU(units=10, input_shape=(3,1),activation='relu')) # Keras GRU 구현에서는 각 게이트별로 두 개의 bias 벡터를 사용해요
