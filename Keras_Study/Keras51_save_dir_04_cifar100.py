...
x_augmented = datagen.flow(
    ...
    save_prefix='cifar100',     # 파일 이름 앞부분 설정 가능
    shuffle=False,
).next()[0]
exit()
...
model = load_model(path+'k38_.hdf5')
#3. 컴파일, 훈련
from tensorflow.keras.callbacks import EarlyStopping,ModelCheckpoint
es = EarlyStopping(monitor='val_loss',mode='min',patience=50,verbose=1,
                   restore_best_weights= True)
hist = model.fit(x_train,y_train, epochs= 500, batch_size= 256,verbose=2,validation_split=0.1, callbacks=[es])
