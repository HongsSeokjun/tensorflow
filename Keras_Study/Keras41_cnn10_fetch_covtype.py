...
x_train = x_train.reshape(-1, 54, 1, 1)  # (batch, height, width, channel)
x_test = x_test.reshape(-1, 54, 1, 1)  # (batch, height, width, channel)



#2. 모델구성
model = Sequential()
model.add(Conv2D(32,(1,1),input_shape=(54, 1, 1), activation='relu'))
...
hist = model.fit(x_train, y_train,epochs= 20, batch_size= 32,verbose=2,validation_split=0.1,callbacks=[es])
