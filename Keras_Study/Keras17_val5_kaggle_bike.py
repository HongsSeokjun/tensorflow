...
hist = model.fit(x_train, y_train, epochs= 100, batch_size= 20,validation_split=0.1)

# 4. 평가, 예측
loss = model.evaluate(x_test, y_test) # 내부적으로 predict를 거쳐서 mse 수식을 거쳐서 나오는 error 값
...
submission_csv.to_csv(path+'submission_0523_1600.csv', index=False)
