...
mcp = ModelCheckpoint(
    ...
    mode='auto',
    save_best_only= True,
    filepath=path+'Cancer_mcp1.hdf5'
)
...
hist = model.fit(x_train, y_train, epochs = 400, batch_size=16, verbose=2, validation_split=0.06, callbacks=[es,mcp],class_weight=class_weights,)

# 중간층 출력 모델 생성 (예: Dense(16, activation='relu') 이후)
# feature_extractor = Model(inputs=model.input, outputs=model.get_layer(index=10).output)

# Feature 추출
# x_train_features = feature_extractor.predict(x_train)
# x_test_features = feature_extractor.predict(x_test)
# test_feat    = feature_extractor.predict(test_csv)
...
# xgb_model.save_model(path +'xgb_25_06_04.json')
# 4. 평가 예측
